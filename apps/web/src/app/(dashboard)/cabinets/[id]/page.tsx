...
import { Building2, MapPin, Globe, Calendar, Users, ExternalLink, ShieldCheck } from 'lucide-react'
import { useAuthStore } from '@/stores/auth'
import { cabinetApi, displayName } from '@/lib/api'
...
export default function CabinetPublicPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['cabinet-public', id, token],
    queryFn: () => cabinetApi.getById(id, token!),
    enabled: !!token,
  })
      ...
      <div className="bg-card border border-border rounded-lg p-6 space-y-4">
        <div className="flex items-start gap-4">
          <div className="h-14 w-14 rounded-xl border border-border bg-muted/30 flex items-center justify-center shrink-0 overflow-hidden">
            {cabinet.logoUrl
              ? <img src={cabinet.logoUrl} alt={cabinet.name} className="h-full w-full object-contain p-1" />
        ...
        {(cabinet.oriasNumber || cabinet.siret) && (
          <div className="flex flex-wrap gap-4 text-xs text-muted-foreground border-t border-border pt-4">
            ...
            {cabinet.siret && (
              <span>
                SIRET : <span className="font-medium text-foreground">{cabinet.siret}</span>
              </span>
            )}
          </div>
        )}
