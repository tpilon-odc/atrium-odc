...
import { publicCabinetApi, type CabinetDirectoryItem } from '@/lib/api'
    ...
    >
      ...
      {cabinet.website && (
        <div
          ...
          onClick={(e) => {
            e.preventDefault()
            window.open(cabinet.website!, '_blank')
          }}
        >
...
export default function AnnuairePage() {
  ...
  const { data, isLoading, isFetching } = useQuery({
    queryKey: ['public-cabinets-directory', search, cursor],
    queryFn: () =>
      publicCabinetApi.list({ search: search || undefined, cursor: cursor ?? undefined, limit: 24 }),
    staleTime: 60_000,
  })
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {Array.from({ length: 9 }).map((_, i) => (
            <div
              key={i}
              className={cn('h-44 bg-muted/40 rounded-xl animate-pulse', i > 5 && 'hidden lg:block')}
            />
          ))}
        </div>
      ) : allItems.length === 0 ? (
